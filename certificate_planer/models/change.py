...
class Change(models.Model):
    ...
    aircraft_type_id = fields.Many2one(
        "certificate_planer.aircraft_type", string="Aircraft Type", tracking=True, ondelete="restrict"
    )
    ...
    status_id = fields.Many2one(
        "certificate_planer.change_status",
        tracking=True,
        default=lambda self: self.env["certificate_planer.change_status"].search([]),
        ondelete="restrict",
    )
    classification_id = fields.Many2one("certificate_planer.change_classification", tracking=True, ondelete="restrict")
    change_id_id = fields.Many2one(
        "certificate_planer.change_id", required=True, string="Change ID", tracking=True, ondelete="restrict"
    )
    ...
    item_ids = fields.One2many(
        "certificate_planer.post_certification_item", "change_id", string="Post Certification Items"
    )
