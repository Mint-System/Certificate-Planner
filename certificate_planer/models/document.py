...
class Document(models.Model):
    ...
    current_revision_id = fields.Many2one(
        ...
        domain="[('document_id','=',id)]",
        tracking=True,
    )
    change_id = fields.Many2one("certificate_planer.change", string="Change", readonly=True)  # deprecated
    type_id = fields.Many2one(
        "certificate_planer.document_type", required=True, string="Type", tracking=True, ondelete="restrict"
    )
    ...
    def store_tpi_report(self):
        ...
        # Return message
        message_id = self.env["certificate_planer.document.message"].create(
            {"message": "The reports have been generated. See attachments of this document."}
        )
        return {
            "name": "Message",
    ...
    def store_mcr_report(self):
        ...
        # Remove existing report attachments
        self.env["ir.attachment"].search([("name", "in", [self.name + ".pdf"])]).unlink()
        ...
        # Return message
        message_id = self.env["certificate_planer.document.message"].create(
            {"message": "The report has been generated. See attachments of this document."}
        )
        return {
            "name": "Message",
            "type": "ir.actions.act_window",
