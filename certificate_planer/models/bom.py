...
class BomPartRel(models.Model):
    ...
    _sql_constraints = [
        (
            "bom_part_unique",
            "UNIQUE (certificate_planer_bom_id, certificate_planer_part_id)",
            "Relation with this BoM and Part already exists.",
        ),
        (
            "self_ref_check",
            "CHECK (certificate_planer_bom_id <> certificate_planer_part_id)",
            "BoM cannot have linked Part as Child Part.",
        ),
    ]
...
class BomPrerequisiteRel(models.Model):
    ...
    _sql_constraints = [
        (
            "bom_prerequisite_unique",
            "UNIQUE (certificate_planer_bom_id, certificate_planer_part_id)",
            "Relation with this BoM and Prerequisite already exists.",
        ),
        (
            "self_ref_check",
            "CHECK (certificate_planer_bom_id <> certificate_planer_part_id)",
            "BoM cannot have linked Part as Prerequisite.",
        ),
    ]
