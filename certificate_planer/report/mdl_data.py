...
def _sort_key(item):
    ...
    return (not name.startswith("135-"), name)
...
def _get_report_values(self, docids, data=None, report_name=""):
    ...
    def get_subparts(level, part):
        ...
        if subparts and part.id not in mainpart_seen:
            parts.append({"part": part, "level": level, "marker": "-" * level, "type": "main", "group": part})
            ...
            if level < 5:
                ...
                for subpart in subparts:
                    ...
                    parts.append(
                        {"part": subpart, "level": level - 1, "marker": "-" * level, "type": "sub", "group": part}
                    )
        ...
        for part in part_ids:
            ...
            for document in part.document_ids:
                ...
                if document.id not in documents_seen:
                    if (
                        report_name == "certificate_planer.mdl_report_view" and document.type_id.class_id.show_on_report
                    ) or (
                        report_name == "certificate_planer.tpi_report_view"
                        and document.type_id.class_id.show_on_tpi_report
                    ):
                        documents_and_parts.append({"doc": document, "parts": parts})
    ...
    tpi_page_text = tpi_page_text.replace("DOCUMENT_NAME", document.name).replace(
        "DOCUMENT_REVISION_TITLE", document.current_revision_id.index_id.name
    )
    ...
    return {
        ...
        "design_organisation_statement_text": self.env["ir.config_parameter"]
        .sudo()
        .get_param("certificate_planer.design_organisation_statement_text"),
    }
